...
`src/core/schema/` 负责合并规则的定义、加载和自动生成。Schema 规则决定每个字段的合并策略（replace / merge / append / smart_match / coerce），是智能合并的核心依据。

## 文件列表

| 文件            | 职责                                       |
| --------------- | ------------------------------------------ |
| `loader.py`     | 从 `schemas/` 目录加载规则，提供字段定义查询 |
| `generator.py`  | 从游戏本体 config 自动生成 schema 规则文件   |
| `dsl.py`        | 识别游戏 DSL key 模式（counter/have/slot 等）|

## 依赖关系

```
infra/types.py --------+
infra/diagnostics.py --+--> loader.py
infra/profiler.py -----+       |
json/classify.py ------+       |
dsl.py ----------------+       |
                                |
...
json/store.py ---------+--> generator.py
json/classify.py ------+       |
dsl.py ----------------+       |
merge/delta.py ---------+  (仅 COMMON_MATCH_KEYS)
```

---

## loader.py — Schema 加载与查询
