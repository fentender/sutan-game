...
`src/core/platform/` 是独立的辅助层，为 core 提供 Steam 本地数据读取、历史版本管理和在线更新检查能力。仅依赖 `infra`，不依赖其他 core 子模块。

## 文件列表

| 文件          | 职责                                           |
| ------------- | ---------------------------------------------- |
| `steam.py`    | Steam ACF/VDF 解析、游戏和 Mod 更新时间读取     |
| `history.py`  | 历史版本目录管理、CAS 布局路径解析               |
| `updater.py`  | 在线更新检查（GitHub / Gitee Releases API）      |

## 依赖关系

```
infra/diagnostics.py --> updater.py
config.py ------------> updater.py (APP_VERSION, UPDATE_SOURCES)
...
找到**早于等于** `mod_update_time` 的最近一个历史版本目录。用于 ADAPTIVE 模式确定 Mod 对应的游戏版本基准。
...
CAS 版本目录通过 `.manifest.json` 查找 hash，定位到 `blobs/{hash[:2]}/{hash[2:]}.json`。非 CAS 版本或游戏本体目录直接返回 `base_dir / rel_path`。
