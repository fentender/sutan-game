...
def resolve_duplicates(
    mod_items: list[tuple[int, JsonObject]],
    base_arr: JsonArray,
    base_indices: list[int],
) -> tuple[list[tuple[int, JsonObject, int]], list[tuple[int, JsonObject]]]:
    """
    ...
    matched_pairs: list[tuple[int, JsonObject, int]] = []
...
def get_key_vals(item: JsonObject, match_keys: list[str]) -> tuple[object, ...] | None:
...
def match_by_keys(
    ...
    for kv, mod_indices in mod_groups.items():
        ...
        base_objs: JsonArray = [base_arr[bi] for bi in base_indices]
