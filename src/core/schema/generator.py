...
_templates_registry: dict[str, JsonObject | None] = {}
...
def _build_element_from_global(arr_canonical: str) -> JsonObject | None:
    """从全局字段信息构建数组元素定义"""
...
def _build_field_from_counts(key: str, type_counts: dict[str, int]) -> JsonObject | None:
...
def _build_template_from_field_info(fi: GlobalFieldEntry) -> JsonObject:
    """从全局字段信息构建命名模板定义。"""
...
def _build_templates() -> dict[str, JsonObject | None]:
...
def _build_dsl_rules() -> dict[str, JsonObject]:
...
def build_field_def(path: str, info: FieldInfo, all_info: dict[str, FieldInfo]) -> JsonObject:
    """构建单个字段的 schema 定义"""
    field_info = info
    ...
    if type_val == "object" or (isinstance(type_val, list) and "object" in type_val):
        ...
        if canonical in _templates_registry:
            ...
            if fields:
                ...
                obj_fields: dict[str, JsonObject] = {
...
def _collect_file_info(filepath: str) -> tuple[str, dict[str, FieldInfo], JsonObject] | None:
...
def _build_single_file_schema(filepath: str, file_type: str, info: dict[str, FieldInfo]) -> JsonObject:
...
def _build_dir_schema(dirpath: str, file_type: str | None, info: dict[str, FieldInfo], file_count: int, total: int) -> JsonObject:
...
def generate_all(config_dir: str, output_dir: str,
    ...
    # 收集根目录文件信息
    file_infos: dict[str, tuple[str, dict[str, FieldInfo], JsonObject]] = {}
