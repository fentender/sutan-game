...
_global_templates: dict[str, JsonObject] = {}
_global_dsl_rules: dict[str, JsonObject] = {}

# get_field_def 查询缓存：(schema_id, field_path_tuple) → result
_field_def_cache: dict[tuple[int, tuple[str, ...]], JsonObject | None] = {}


@profile
def load_schemas(schema_dir: Path | str) -> dict[str, JsonObject]:
    """
    ...
    for schema_file in schema_dir.glob("*.schema.json"):
        ...
        schema: JsonObject = json.loads(raw)
...
def resolve_schema(
    rel_path: str,
    schemas: dict[str, JsonObject] | None = None,
) -> JsonObject | None:
    """
    根据文件相对路径匹配 schema。
...
def _resolve_template(template_name: str) -> JsonObject | None:
    """从全局模板中解析命名模板"""
...
def get_schema_root_key(schema: JsonObject) -> str:
