...
- 查找定义、引用、实现等代码导航任务时，必须优先使用 LSP 工具，不要用 Grep 代替
- LSP 的 `character` 参数必须对准符号文本所在列，不能用 1 或行首位置——行首通常是缩进空白，会导致查找失败
- 定位方法：先看目标行内容，数出符号起始字符的列号（1-based），用该值作为 `character`
